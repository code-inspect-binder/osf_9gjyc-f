...
## 20 trials, 5 easy rounds (90 balls of one color); 15 hard rounds (50 balls of one color)
trials <- c(rep(90,5),rep(50,15))
trials

# switching matrix (based on pilot + exp 1 + exp 2)
p_switch <- read.csv("p_switch_ind_maj.csv", header = TRUE)
p_switch
...
for(gsize in gsizes)
  { 
  p_switch_sel <- subset(p_switch, Group.size == gsize) 
  
  for (runs in 1:total)
        {
        # shuffle order of trials  
        all.tr <- sample(trials)
        ...
        for (TRIAL in all.tr)
            ...
            if(TRIAL.NR > 1)
              {
              # look up p(switch) in table based on reward (yes/no) and confidence unselected advisor
              sel.row <- subset(p_switch_sel, Reward == reward &  Conf.unsel.adv == unselect.adv.conf)
              sel.row
              # Change here depending on single versus group transition probability
                ...
                HA_confidence    <- ifelse(HA_evidence < 0.10, 5, 
                    ...
                    unselect.adv.conf <- ifelse(sel.adv == "HA", SA_confidence, HA_confidence)
                        ...
                        reward <- ifelse(Client_decision == outcome, 1, 0)
                            ...
                            # store results
                            outcomes <- data.frame(reward, sel.adv, TRIAL.NR, gsize)
          ...
          # save results
          write.table(results, "Runs majority vote.txt", col.names= FALSE, append = TRUE)
  }
}